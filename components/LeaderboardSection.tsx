...
function LeaderboardContent({ onCollapse }: LeaderboardSectionProps) {
  ...
  return (
    <section style={{ display: 'flex', flexDirection: 'column', width: '100%', marginTop: '16px' }}>
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column' }}>
        {/* 상단 접기 버튼 (onCollapse 콜백이 전달된 경우 노출) */}
        {onCollapse && (
        ...
        {/* 하단 접기 버튼 (onCollapse 콜백이 전달된 경우 노출) */}
        {onCollapse && (
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '14px', marginBottom: '10px' }}>
            <button
              onClick={onCollapse}
              style={{
                display: 'inline-flex',
